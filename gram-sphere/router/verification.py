...
async def upload_work_evidence(
    ...
    with engine.connect() as conn:
        skill_row = conn.execute(text("SELECT skill_type, proficiency_level FROM user_skills WHERE user_id = :id AND is_primary_skill = TRUE"), {"id": user_id}).fetchone()
...
async def upload_skill_task(
    ...
    with engine.connect() as conn:
        user = conn.execute(text("SELECT current_district FROM users WHERE id = :id"), {"id": user_id}).fetchone()
    registered_city = user.current_district + ", Karnataka"
    ...
    with engine.connect() as conn:
        ...
        if ai_passed:
            ...
            conn.execute(text("UPDATE users SET skill_tokens = skill_tokens + :gain WHERE id = :id"), {"gain": token_gain, "id": user_id})
...
async def get_work_history(user_id: str):
    """Returns a list of past work experience and verified portfolio entries."""
    with engine.connect() as conn:
        # Employment records
        jobs = conn.execute(text("SELECT job_title, start_date, end_date, client_rating FROM employment_records WHERE user_id = :id"), {"id": user_id}).fetchall()
        
        # Verified work samples
        samples = conn.execute(text("SELECT work_description, ai_overall_score, created_at FROM skill_media WHERE user_id = :id AND status = 'verified'"), {"id": user_id}).fetchall()
...
async def get_trust_score(user_id: str):
    ...
    with engine.connect() as conn:
        # 1. Portfolio Score
        samples = conn.execute(text("SELECT ai_overall_score FROM skill_media WHERE user_id = :id AND status = 'verified'"), {"id": user_id}).fetchall()
        ...
        # 2. Badges Score
        badges = conn.execute(text("SELECT COUNT(*) FROM skill_badges WHERE user_id = :id AND is_valid = TRUE"), {"id": user_id}).fetchone()[0]
        # Assume 10 badges = max score for this component
        badge_component = min((badges / 10) * 30, 30)
        
        # 3. Experience Score
        exp = conn.execute(text("SELECT years_of_experience FROM user_skills WHERE user_id = :id AND is_primary_skill = TRUE"), {"id": user_id}).fetchone()
        years = exp.years_of_experience if exp else 0
        rating_row = conn.execute(text("SELECT AVG(client_rating) FROM employment_records WHERE user_id = :id"), {"id": user_id}).fetchone()
        ...
        # Update the database
        conn.execute(text("UPDATE users SET trust_score = :score WHERE id = :id"), {"score": round(final_score, 1), "id": user_id})
        conn.commit()
